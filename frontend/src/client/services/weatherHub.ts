...
export class WeatherHub {
  ...
  ): Promise<WeatherResult<WeatherGetTwoHourForecastResponse | WeatherGetTwentyFourHourForecastResponse | WeatherGetFourDayOutlookResponse>> {
    try {
      let data = null;
      ...
      switch (type) {
        ...
        case ForecastType.TwentyFourHour:
          data = await WeatherService.getTwentyFourHourForecast(date, paginationToken);
          break;
  ...
  static async getWeatherData(
    ...
    date?: string,
    paginationToken?: string
  ): Promise<WeatherResult<WeatherGetAirTemperatureResponse | WeatherGetWindDirectionResponse | WeatherGetLightningResponse | WeatherGetWbgtResponse>> {
    try {
      let data = null;
  ...
  static async getWeatherDashboard() {
    try {
      ...
      const [twoHourResult, airTempResult, windDirectionResult] = await Promise.all([
      ...
      if (!twoHourResult.success || !airTempResult.success || !windDirectionResult.success) {
        const errors = [
          twoHourResult.error,
          airTempResult.error,
   ...
   */
  static getUniqueAreas(forecasts: Forecast[], includeAllAreas: boolean = true): string[] {
